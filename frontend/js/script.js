...
function searchWeather(eventOrCity = null) {
  const city =
    eventOrCity && typeof eventOrCity === "object"
  ...
  fetch(
    `https://neoweather-api.lucas-dev.workers.dev/clima?cidade=${encodeURIComponent(city)}`,
  )
    .then((response) =>
      !response.ok ? Promise.reject("Cidade não encontrada") : response.json(),
    )
    .then(displayWeather)
    .catch(
      (error) =>
